    ...
    public static class CollisionDetector
    {
        ...
        public static void BoxAndBox(CollisionBox one, CollisionBox two, CollisionData data)
        {
            ...
            else
            {
                ...
                if (one.Body.FreezeRotation)
                {
                    for (int i = 0; i < 8; i++)
                    {
                        // Calculate the position of each vertex
                        _vertexPos = new Vector3d(mults[i, 0], mults[i, 1], mults[i, 2]);
                        _vertexPos *= two.HalfSize;
                        _vertexPos = two.Transform.Transform(_vertexPos);
                        ...
                        {
                            var contact = data.GetContact();
                            contact.ContactPoint = Vector3d.UnitY;
                            contact.ContactPoint *= (vertexDistance - one.Body.offset);
                            contact.ContactPoint += _vertexPos;
                            contact.ContactNormal = Vector3d.UnitY;
                            contact.Penetration = one.Body.offset - vertexDistance;
                ...
                else if (two.Body.FreezeRotation)
                {
                    for (int i = 0; i < 8; i++)
                    {
                        // Calculate the position of each vertex
                        _vertexPos = new Vector3d(mults[i, 0], mults[i, 1], mults[i, 2]);
                        _vertexPos *= one.HalfSize;
                        _vertexPos = one.Transform.Transform(_vertexPos);
                        ...
                        if (vertexDistance <= two.Body.offset)
                        {
                            var contact = data.GetContact();
                            contact.ContactPoint = Vector3d.UnitY;
                            contact.ContactPoint *= (vertexDistance - two.Body.offset);
                            contact.ContactPoint += _vertexPos;
                            contact.ContactNormal = Vector3d.UnitY;
                            contact.Penetration = two.Body.offset - vertexDistance;
