...
namespace Cyclone.Rigid.Constraints
{
    ...
    public class RigidContact
    {
        ...
        public double speedEpsilon = 0.01f;
        public double rotationEpsilon = 1f;
        ...
        public void ApplyVelocityChange(Vector3d[] velocityChange, Vector3d[] rotationChange)
        {
            ...
            // Apply the changes
            Body[0].Velocity += Body[0].FreezeRotation ? Vector3d.Zero : velocityChange[0];
            Body[0].Rotation += Body[0].FreezeRotation ? Vector3d.Zero : rotationChange[0];
            ...
            if (Body[1] != null)
            {
                ...
                // And apply them.
                Body[1].Velocity += Body[1].FreezeRotation ? Vector3d.Zero : velocityChange[1];
                Body[1].Rotation += Body[1].FreezeRotation ? Vector3d.Zero : rotationChange[1];
            }
        }
        ...
        public void ApplyPositionChange(Vector3d[] linearChange, Vector3d[] angularChange, double penetration)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                if ((angularMove[i] == 0 )|| Body[i].FreezeRotation)
                {
                ...
                Body[i].Position = Body[i].FreezeRotation ? Body[i].Position : pos;
